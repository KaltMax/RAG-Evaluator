  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-400">Upload a JSON file to create a batch experiment</p>
      </div>
      ...
      <div className="bg-[#2D2D2D] rounded-lg shadow-lg p-6">
        ...
        >
          ...
          <div className="min-h-[56px] flex flex-col justify-center">
            {isDragActive ? (
              <p className="text-lg text-blue-400">Drop the JSON file here...</p>
            ) : (
              <>
                <p className="text-lg text-gray-300 mb-2">
                  Drag and drop a JSON experiment file here, or click to select
                </p>
                <p className="text-sm text-gray-500">Only .json files are supported</p>
              </>
            )}
          </div>
        </div>
            ...
            <div className="bg-[#1F1F1F] rounded-lg p-4 border border-gray-700">
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-white font-medium mt-1">{experimentData.Name}</p>
                </div>
                <div>
                  <span className="text-gray-400">Repeat Count</span>
                  <p className="text-white font-medium mt-1">{experimentData.RepeatCount}</p>
                </div>
                <div>
                  <span className="text-gray-400">Queries</span>
                  <p className="text-white font-medium mt-1">{experimentData.Queries.length}</p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {experimentData.Queries.map((query, index) => (
                    <tr key={`${query.Question}-${query.Language}`} className="border-b border-gray-700/50 last:border-b-0">
            ...
            {result.success ? (
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-green-400 font-medium">Experiment Created</span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <span className="text-gray-400">Experiment ID:</span>
                  <span className="text-gray-200 font-mono text-xs">{result.data.id}</span>
                  <span className="text-gray-400">Name:</span>
